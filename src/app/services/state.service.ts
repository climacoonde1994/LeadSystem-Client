...
function matches(item: any, term: string, pipe: PipeTransform) {
  return item.name.toLowerCase().includes(term.toLowerCase()) || item.code.toLowerCase().includes(term.toLowerCase());
}
...
export class StateService {
  ...
  public loadList = (id: number) => {
    this.getListByStateId(id).subscribe(response => {
      this.$search.pipe(
  ...
  public getByCode = (countrycode: string , statecode : string) => {
    return this.repositoryHelper.get('api/state/getByCode?countrycode=' + countrycode + '&statecode='+statecode);
  }

  public getListByStateId = (id: number) => {
    return this.repositoryHelper.get('api/state/getList?id=' + id);
  }

  public getById = (id: number) => {
    return this.repositoryHelper.get('api/state/getById?id=' + id);
  }

  public create = (body: any) => {
    return this.repositoryHelper.post('api/state/create', body);
  }

  public update = (body: any) => {
    return this.repositoryHelper.put('api/state/update', body);
  }

  public delete = (id: number) => {
    return this.repositoryHelper.delete('api/state/delete?id=' + id);
  }

  public toggle = (id: number) => {
    return this.repositoryHelper.put('api/state/toggle?id=' + id, null);
  }
